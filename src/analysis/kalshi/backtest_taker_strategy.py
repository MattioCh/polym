"""Backtest: selective contrarian taker strategy with monthly parameter recalculation.

The market is structurally hostile to takers (−1.5 to −2.5% excess return overall).
This strategy survives by being **extremely selective**, trading only in the narrow
pockets where takers historically break even or profit:

1. **NO-side contrarian bias**: YES-optimism tax means NO is positive EV at most
   price levels. The strategy overwhelmingly buys NO.
2. **Near-close timing**: Maker edge collapses to 0.67% in the last hour — this
   is when taker disadvantage is smallest and price discovery is most complete.
3. **Category filtering**: Only trade in categories/buckets where historical
   taker excess is positive (data-driven, re-estimated each period).
4. **Conservative sizing**: Position sizes are ~50% of the maker strategy
   defaults, with tighter risk limits.

Walk-forward temporal separation (identical to maker backtest):
...
- PnL attributed to market close date (resolution date).

Key taker-favorable niches from the analysis:
- Esports: the only category with negative maker / positive taker excess
- Finance: near-efficient (0.17pp gap), taker losses are minimal
- Near-close (<1h): maker edge drops sharply, prices are near-terminal
- NO side at prices ≤ 55¢: YES-optimism tax creates persistent NO value
- Weekday mid-week (Wed): worst day for makers (+1.06%), best chance for takers
- Large orders (500+): less adversely selected (−1.54% vs −2.47%)
"""
...
# NOTE: Much more conservative than the maker strategy.
DEFAULT_CONFIG = {
    ...
    # Minimum taker excess (%) in a bucket to participate.
    "min_taker_excess_pct": 0.5,
    ...
    # Near-close (< 1h) BOOST — maker edge weakest here
    "near_close_size_mult": 1.5,
    # Short-horizon (1-6h) slight boost
    "short_horizon_size_mult": 1.25,
    # Maker sweet-spot (6h-7d) REDUCE — maker edge strongest here
    "maker_sweet_spot_size_mult": 0.5,
    # Long duration (30d+) moderate
    "long_duration_size_mult": 0.75,
    # Weekend REDUCE — weekends amplify maker edge
    "weekend_size_mult": 0.5,
    # Mid-week (Wednesday) boost — worst maker day
    "midweek_size_mult": 1.25,
    # NO-side alignment boost / YES-side penalty in NO-zone
    "no_side_boost": 1.25,
    "yes_penalty_in_no_zone": 0.5,
    # Minimum months of historical data before first trade
    "min_warmup_months": 6,
    # ── Capital management ────────────────────────────────────────────────
    # Initial capital in cents ($10,000 = 1,000,000 cents)
    "initial_capital": 1_000_000,
    # Maximum fraction of equity in a single trade (tighter than maker)
...
class BacktestTakerStrategyAnalysis(Analysis):
    """Walk-forward backtest of a selective contrarian taker strategy."""
    ...
    def __init__(
        ...
        super().__init__(
            name="backtest_taker_strategy",
            description="Walk-forward backtest of selective contrarian taker strategy with monthly parameter recalculation",
        )
    ...
    def _run_impl(self) -> AnalysisOutput:
        ...
        with self.progress(f"Running walk-forward taker backtest ({len(recalc_dates)-1} periods)"):
        ...
        output_data["strategy"] = "selective_taker"
    ...
    # ── Parameter estimation (TAKER perspective) ─────────────────────────────

    def _estimate_parameters(
        self, con: duckdb.DuckDBPyConnection, cutoff: pd.Timestamp
    ) -> dict:
        """Compute taker-side strategy parameters from data strictly before cutoff.
        ...
        Returns:
        - group_taker_edge: {group: taker_excess_pct}
        - composite_taker_edge: {(group, price_bucket, time_bucket, day_type): taker_excess_pct}
        - no_side_threshold: price below which NO side has higher taker EV
        """
        ...
        # ── Composite taker edge table (group × price × time × day) ─────────
        df = con.execute(f"""
            WITH trade_data AS (
                SELECT
                    ...
                    END AS price_bucket,
                    CASE
                        WHEN EXTRACT(EPOCH FROM (m.close_time - t.created_time)) / 3600.0 <= 1 THEN '0-1h'
                        WHEN EXTRACT(EPOCH FROM (m.close_time - t.created_time)) / 3600.0 <= 6 THEN '1-6h'
                        WHEN EXTRACT(EPOCH FROM (m.close_time - t.created_time)) / 3600.0 <= 72 THEN '6h-3d'
                        ELSE '3d+'
                    END AS time_bucket,
                    -- TAKER PnL (opposite sign of maker PnL)
                    CASE
                    ...
                    END AS taker_pnl,
                    -- Taker cost basis (what the taker pays)
                    CASE
                        WHEN t.taker_side = 'yes' THEN t.yes_price * t.contracts
                        ELSE t.no_price * t.contracts
                    END AS taker_cost
                FROM trades t
                INNER JOIN markets m ON t.ticker = m.ticker
        ...
        if df.empty:
            return {"group_taker_edge": {}, "composite_taker_edge": {}, "no_side_threshold": 55}
        ...
        # Group-level taker edge
        group_agg = (
            df.groupby("group")
            .agg({"taker_pnl": "sum", "taker_cost": "sum", "total_contracts": "sum"})
            .reset_index()
        )
        group_agg["taker_excess_pct"] = (
            group_agg["taker_pnl"] * 100.0 / group_agg["taker_cost"].replace(0, np.nan)
        )
        group_taker_edge = dict(zip(group_agg["group"], group_agg["taker_excess_pct"]))

        # Composite taker edge (per bucket)
        combo = (
            df.groupby(["group", "price_bucket", "time_bucket", "day_type"])
            .agg({"taker_pnl": "sum", "taker_cost": "sum", "total_contracts": "sum"})
            .reset_index()
        )
        combo["taker_excess_pct"] = (
            combo["taker_pnl"] * 100.0 / combo["taker_cost"].replace(0, np.nan)
        )
        min_contracts = self.config["min_bucket_contracts"]
        combo = combo[combo["total_contracts"] >= min_contracts]

        composite_taker_edge: dict[tuple, float] = {}
        for _, row in combo.iterrows():
            key = (row["group"], row["price_bucket"], row["time_bucket"], row["day_type"])
            composite_taker_edge[key] = row["taker_excess_pct"]

        # ── Directional bias: find NO-side threshold for takers ──────────────
        # NO is almost always better for takers. Find where the advantage flips.
        ...
        no_side_threshold = 55  # default: lean NO below 55
        if not dir_df.empty:
            pivoted = dir_df.pivot_table(
                index="price_mid",
                columns="taker_side",
                values=["taker_pnl", "taker_cost"],
                aggfunc="sum",
            ).fillna(0)
            if ("taker_pnl", "no") in pivoted.columns and ("taker_pnl", "yes") in pivoted.columns:
                # Taker NO EV: when taker_side='no'
                taker_no_excess = (
                    pivoted[("taker_pnl", "no")] * 100.0
                    / pivoted[("taker_cost", "no")].replace(0, np.nan)
                )
                # Taker YES EV: when taker_side='yes'
                taker_yes_excess = (
                    pivoted[("taker_pnl", "yes")] * 100.0
                    / pivoted[("taker_cost", "yes")].replace(0, np.nan)
                )
                # Find highest price where NO taker still outperforms YES taker
                for pm in sorted(pivoted.index):
                    no_e = taker_no_excess.get(pm, 0)
                    yes_e = taker_yes_excess.get(pm, 0)
                    if pd.notna(no_e) and pd.notna(yes_e) and no_e > yes_e:
                        no_side_threshold = pm + 10

        return {
            "group_taker_edge": group_taker_edge,
            "composite_taker_edge": composite_taker_edge,
    ...
    def _walk_forward(
        ...
        for i in trange(len(recalc_dates) - 1, desc="Estimating parameters"):
            ...
            n_positive_buckets = sum(
                1 for v in params["composite_taker_edge"].values()
                if v > cfg["min_taker_excess_pct"]
            )
            param_history.append({
                "period_start": period_start,
                "period_end": period_end,
                "n_groups_positive_taker": sum(
                    1 for v in params["group_taker_edge"].values() if v > 0
                ),
                "n_composite_buckets_positive": n_positive_buckets,
                "n_composite_buckets_total": len(params["composite_taker_edge"]),
                "no_side_threshold": params["no_side_threshold"],
                "avg_group_taker_edge": (
                    np.mean(list(params["group_taker_edge"].values()))
                    if params["group_taker_edge"] else 0
                ),
            })

            if not params["composite_taker_edge"]:
                continue
        ...
        for day_ts in date_range:
            ...
            max_per_day = cfg.get("max_trades_per_day", 30)
        ...
        return con.execute(f"""
            SELECT
                ...
                CASE
                    ...
                    ELSE 'Weekday'
                END AS day_type,
                -- Day-of-week number (0=Mon ... 6=Sun in DuckDB iso)
                dayofweek(t.created_time) AS dow,
                CASE
                    WHEN t.yes_price BETWEEN 1 AND 20 THEN '01-20'
                    WHEN t.yes_price BETWEEN 21 AND 40 THEN '21-40'
                    WHEN t.yes_price BETWEEN 41 AND 60 THEN '41-60'
    ...
    def _filter_trades(self, trades: pd.DataFrame, params: dict) -> pd.DataFrame:
        """Apply extreme selectivity filters and compute effective_edge.

        Does NOT size positions — sizing is done in _walk_forward with capital
        awareness.  Returns filtered trades with:
        - composite_taker_edge: raw bucket edge from parameter estimation
        - effective_edge: composite_taker_edge × timing/day/direction adjustments
        ...
        # ── Filter 3: Composite bucket must have POSITIVE taker excess ───────
        composite_taker_edge = params["composite_taker_edge"]
        min_excess = cfg["min_taker_excess_pct"]

        def lookup_edge(row):
            key = (row["group"], row["price_bucket"], row["time_bucket"], row["day_type"])
            return composite_taker_edge.get(key, np.nan)

        trades["composite_taker_edge"] = trades.apply(lookup_edge, axis=1)
        trades = trades[
            trades["composite_taker_edge"].notna()
            & (trades["composite_taker_edge"] > min_excess)
        ]
        if trades.empty:
            return trades
        ...
        trades["pnl_per_contract"] = trades["taker_pnl"] / safe_contracts
        trades["cost_per_contract"] = trades["taker_cost"] / safe_contracts

        # ── Effective edge = composite_taker_edge × adjustments ──────────────
        edge_mult = np.ones(len(trades))

        # Time-to-close (INVERTED from maker — boost near close, reduce sweet spot)
        htc = trades["hours_to_close"].values
        edge_mult = np.where(htc < 1, edge_mult * cfg["near_close_size_mult"], edge_mult)
        edge_mult = np.where(
            (htc >= 1) & (htc < 6), edge_mult * cfg["short_horizon_size_mult"], edge_mult
        )
        edge_mult = np.where(
            (htc >= 6) & (htc <= 168), edge_mult * cfg["maker_sweet_spot_size_mult"], edge_mult
        )
        ...
        # Mid-week boost (Wednesday = worst maker day)
        is_wednesday = trades["dow"].values == 3
        edge_mult = np.where(is_wednesday, edge_mult * cfg["midweek_size_mult"], edge_mult)

        # Directional alignment
        no_threshold = params["no_side_threshold"]
        yes_price = trades["yes_price"].values
        taker_side = trades["taker_side"].values
        in_no_zone = yes_price <= no_threshold

        # NO-side alignment boost
        aligned_no = in_no_zone & (taker_side == "no")
        edge_mult = np.where(aligned_no, edge_mult * cfg["no_side_boost"], edge_mult)

        # YES-side penalty in NO zone
        misaligned_yes = in_no_zone & (taker_side == "yes")
        edge_mult = np.where(misaligned_yes, edge_mult * cfg["yes_penalty_in_no_zone"], edge_mult)

        trades["effective_edge"] = trades["composite_taker_edge"].values * edge_mult
    ...
    def _build_trade_log(self, trade_results: pd.DataFrame) -> pd.DataFrame:
        ...
        # Dollar columns
        log["adj_taker_pnl_dollars"] = log["adj_taker_pnl"] / 100.0
        log["adj_taker_cost_dollars"] = log["adj_taker_cost"] / 100.0
        ...
        cols = [
            ...
            "taker_side", "result", "taker_won",
            "yes_price", "no_price",
            "contracts", "sized_contracts", "participation_rate",
            "taker_pnl", "adj_taker_pnl", "adj_taker_pnl_dollars",
            "taker_cost", "adj_taker_cost", "adj_taker_cost_dollars",
            "balance_before", "balance_before_dollars",
            "balance_after", "balance_after_dollars",
            "open_exposure", "open_exposure_dollars",
            "effective_edge", "composite_taker_edge",
    ...
    def _compute_results(
        ...
        daily_pnl = (
            trade_results.groupby("pnl_date")["adj_taker_pnl"]
            .sum()
        )
        ...
        total_capital = trade_results["adj_taker_cost"].sum()
        trade_wins = int(trade_results["taker_won"].sum())
    ...
    def _monthly_breakdown(self, trade_results: pd.DataFrame) -> pd.DataFrame:
        ...
        monthly = (
            tr.groupby("month")
            .agg(
                pnl=("adj_taker_pnl", "sum"),
                cost=("adj_taker_cost", "sum"),
                trades=("adj_taker_pnl", "count"),
                wins=("taker_won", "sum"),
            )
            .reset_index()
        )
    ...
    def _create_figure(
        ...
        color = "#3498db"  # blue for taker (vs green for maker)
        ...
        ax1.set_title("Equity Curve (Selective Taker Strategy)", fontsize=14, fontweight="bold")
        ...
        if not monthly_df.empty:
            colors = ["#3498db" if x >= 0 else "#e74c3c" for x in monthly_df["pnl_dollars"]]
        ...
        if param_history:
            ...
            ax6_twin = ax6.twinx()
            ax6.plot(
                ph["period_start"], ph["n_composite_buckets_positive"],
                "o-", color="#e67e22", markersize=3, label="Positive taker buckets",
            )
            ax6_twin.plot(
                ph["period_start"], ph["avg_group_taker_edge"],
                "s--", color="#9b59b6", markersize=3, label="Avg group taker edge %",
            )
            ax6.set_title("Taker Opportunity Over Time")
            ax6.set_ylabel("# Positive Buckets", color="#e67e22")
            ax6_twin.set_ylabel("Avg Taker Edge %", color="#9b59b6")
            ax6.grid(alpha=0.3)
            ax6.tick_params(axis="x", rotation=45)
            lines1, labels1 = ax6.get_legend_handles_labels()
            lines2, labels2 = ax6_twin.get_legend_handles_labels()
            ax6.legend(lines1 + lines2, labels1 + labels2, loc="upper left", fontsize=7)
        else:
            ax6.text(0.5, 0.5, "No parameter history", ha="center", va="center")

        plt.suptitle(
            f"Selective Taker Strategy Backtest — {self.config['recalc_frequency'].title()} Recalculation",
    ...
    def _create_chart(self, rolling: pd.DataFrame, metrics: BacktestMetrics) -> ChartConfig:
        ...
        return ChartConfig(
            ...
            xKey="date",
            yKeys=["equity"],
            title=f"Selective Taker Strategy — Sharpe {metrics.sharpe_ratio:.2f}",
